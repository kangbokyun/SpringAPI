...
public class MelonNewChart {
        @Autowired static NewSongService newSongService;
        @Autowired static NewSongRepository newSongRepository;
...
//			System.out.println(songLike.text());
                        ...
                        ArrayList<NewSongDTO> songDTOS = newSongService.getNewSong();

                        List<NewSongEntity> result = newSongRepository.findAll();
                        for(int i = 0; i < melon_song.size(); i++) {
                                System.out.println("newSongList : " + songDTOS.get(i).getS_title());
